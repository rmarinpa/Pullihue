Public Class frm_ticket_historial
    ...
    Private Sub frm_historial_ticket_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not txt_id_solicitud.Text.Trim().Equals("") Then
            ...
            If dgv_ticket.RowCount > 0 Then
                txt_comentario.Text = dgv_historial_ticket.Rows(0).Cells("comentario").Value
                'AsignarTicket()
            End If
        End If
    End Sub
    ...
    Private Sub AsignarTicket()
        Select Case dgv_ticket.Rows(0).Cells(3).Value.ToString()
            Case 0
                txt_estado.Text = "Cerrado"
            Case 1
                txt_estado.Text = "Abierto"
        End Select
        txt_asunto.Text = dgv_ticket.Rows(0).Cells(1).Value.ToString()
        dtp_ingreso.Text = dgv_ticket.Rows(0).Cells(4).Value.ToString()
        If Not dgv_ticket.Rows(0).Cells(5).Value.ToString().Equals("") Then
            dtp_finaliza.Text = CDate(dgv_ticket.Rows(0).Cells(5).Value)
        Else
            dtp_finaliza.Text = "1900-01-01"
        End If
        txt_nombre.Text = dgv_ticket.Rows(0).Cells(6).Value.ToString()
        If dgv_ticket.Rows(0).Cells(7).Value.ToString().Equals("Seleccione") Then
            txt_prioridad.Text = "Sin asignar"
        Else
            txt_prioridad.Text = dgv_ticket.Rows(0).Cells(7).Value.ToString()
        End If
        BuscarHistorial(id)
    End Sub
    ...
    Private Sub btn_nuevo_registro_Click(sender As Object, e As EventArgs) Handles btn_nuevo_registro.Click
        ...
        Else
            frm_ticket_nuevo.txt_solicitud_id.Text = txt_id_solicitud.Text.Trim()
            frm_ticket_solicitud.abrir_en_panel(frm_ticket_nuevo, frm_ticket_solicitud.pnl_body)
            With frm_ticket_solicitud.lbl_titulo
                .Text = "Agregar historial a " & txt_nombre.Text
