Public Class frm_modificar_ticket

    Dim nuevo_ticket As New clases.ticket

    Dim prioridad, nombre As String
    ...
    Private Sub Desbloqueo()
        If frm_solicitud_ticket.lbl_solicitante.Text.Equals("GSOLIS") Then
    ...
    Private Sub btn_buscar_Click(sender As Object, e As EventArgs) Handles btn_buscar.Click
        ...
        If (Not nombre.Equals("")) Then
            dgv_ticket.DataSource = nuevo_ticket.BuscarTicket(nombre)
            OrdenColumnasData()
        End If
        Bloqueado()
    End Sub
    ...
    Private Sub Asignar(fila)
        prioridad = dgv_ticket.Rows(fila).Cells(5).Value.ToString()
        nombre = dgv_ticket.Rows(fila).Cells(4).Value.ToString()
        If (dgv_ticket.Rows(fila).Cells(1).Value.ToString().Equals("1")) Then
            estado = True
        Else
            estado = False
        End If
        fecha_ingreso = CDate(dgv_ticket.Rows(fila).Cells(2).Value)
        If Not dgv_ticket.Rows(fila).Cells(3).Value.ToString().Equals("") Then
            fecha_final = CDate(dgv_ticket.Rows(fila).Cells(3).Value)
        Else
    ...
    Private Sub AsignarParametros()
        frm_historial_ticket.txt_id_solicitud.Text = ident
        'frm_historial_ticket.chb_nueva_busqueda.Checked = False
        frm_solicitud_ticket.abrir_en_panel(frm_historial_ticket, frm_solicitud_ticket.pnl_body)
        'If estado Then
