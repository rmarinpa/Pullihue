...
Public Class guias_despacho
    ...
    Public Function listar6() As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM folio4 order by folio desc limit 3", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar6 = nue_var.dt
        Return listar6
    ...
    Public Function listar17() As DataTable
        ...
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM val_monto order by Id_notas_de_pedido asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar17 = nue_var.dt
        Return listar17
    ...
    Public Function listar24(ByVal Codigo As String) As DataTable
        ...
        Return listar24
    End Function
    Public Sub insertar7(ByVal folio As Double, ByVal contacto As String, ByVal medio As String)
        nue_var.consulta = "INSERT INTO contacto_venta VALUES('', ?folio, ?contacto, ?medio)"
        Try
            nue_conexion.desconectar()
            nue_conexion.conectar()
            nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
        ...
        End Try
    End Sub
    Public Function descripcion_fecha(ByVal rut As String, ByVal numero_pedido As Double) As Date
        nue_var.consulta = "SELECT fecha_act FROM consulta_factura where rut = ?rut and numero_pedido = ?numero_pedido"
        nue_conexion.conectar()
        nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
        nue_var.cmd.Parameters.Add("?rut", MySql.Data.MySqlClient.MySqlDbType.VarChar).Value = rut
        nue_var.cmd.Parameters.Add("?numero_pedido", MySql.Data.MySqlClient.MySqlDbType.Double).Value = numero_pedido
        nue_var.dr = nue_var.cmd.ExecuteReader()
        If nue_var.dr.Read Then
            If IsDBNull(nue_var.dr("fecha_act")) Then
                descripcion_fecha = ""
            Else
                descripcion_fecha = nue_var.dr("fecha_act")
            End If
        End If
        nue_var.dr.Close()
        nue_conexion.desconectar()
    ...
    End Function
    Public Function listar25(ByVal rut As String) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT distinct(numero_pedido) FROM dif_producto where rut = '" & rut & "' AND Pend <> 0 order by numero_pedido desc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar25 = nue_var.dt
        Return listar25
    ...
    Public Function consultar_cliente(ByVal rut As String) As Integer
        nue_var.consulta = "SELECT Pend FROM dif_producto WHERE rut = ?rut and Pend > 0"
        nue_conexion.conectar()
        nue_var.cmd = New MySqlCommand(nue_var.consulta, nue_conexion.conex())
        nue_var.cmd.Parameters.Add("?rut", MySql.Data.MySqlClient.MySqlDbType.VarChar).Value = rut
        nue_var.dr = nue_var.cmd.ExecuteReader()
    ...
    End Function
    Public Function listar27(ByVal rut As String) As DataTable
        nue_var.dt = New DataTable
        nue_conexion.conectar()
        nue_var.da = New MySqlDataAdapter("SELECT * FROM dif_producto where rut = '" & rut & "' and CAST(numero_pedido AS UNSIGNED) <> 0 and Pend > 0 order by numero_pedido asc", nue_conexion.conex())
        nue_var.da.Fill(nue_var.dt)
        nue_conexion.desconectar()
        listar27 = nue_var.dt
        Return listar27
